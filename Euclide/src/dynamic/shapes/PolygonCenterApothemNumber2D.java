/**
 * File: 	PolygonCenterApothemNumber2D.java
 * Project: Euclide
 * 
 ...
 *
 */
public class PolygonCenterApothemNumber2D extends DynamicShape2D {
	 ...
	 * @param point
	 */
	public PolygonCenterApothemNumber2D(DynamicShape2D point1, 
	...
	public void update() {
		...
		double theta0 = Angle2D.getHorizontalAngle(center, apoth) + dTheta/2;
		double dist = center.getDistance(apoth);
		double diag = dist/Math.cos(Math.PI/vertexNumber);
		
		// build the polygon vertices
		Point2D[] vertices = new Point2D[vertexNumber];
		for (int i=0; i<vertexNumber; i++){
			vertices[i] = Point2D.createPolar(center, diag, theta0+i*dTheta);
		}

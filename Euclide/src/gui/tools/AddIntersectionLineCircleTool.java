/* file : AddIntersectionLineCircleTool.java
 * 
 * Project : Euclide
 *
 * ===========================================
...
import dynamic.shapes.IntersectionLineCircle2D;

/**
 * Compute intersection between a line and a circle or circle arc.
 * @author dlegland
 */
public class AddIntersectionLineCircleTool extends EuclideTool {

	//StraightObject2D line=null;
	DynamicShape2D line = null;
	
	public AddIntersectionLineCircleTool(EuclideGui gui, String name){
		super(gui, name);		
	...
	public void mousePressed(MouseEvent evt) {
		...
		if(step==1){
			// pick a line
			elem = view.getSnappedShape(point, LinearShape2D.class);
			if(elem==null) return;
 			line = elem.getGeometry();
			step = 2;
		}else{
			// pick a circular shape
			elem = view.getSnappedShape(point, CircularShape2D.class);
			if(elem==null) return;
 			DynamicShape2D circle = elem.getGeometry();

 			// create the intersections
 			IntersectionLineCircle2D p1 =
 				new IntersectionLineCircle2D(line, circle, 0);
 			IntersectionLineCircle2D p2 = 
	...
	public void mouseMoved(MouseEvent evt){		
		...
		if(step==1){
			...
			EuclideFigure item = view.getSnappedShape(point, 
					LinearShape2D.class);
